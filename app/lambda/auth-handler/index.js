...
function getAuthToken(headers, event) {
    ...
    if (parts.length !== 2 || parts[0] !== 'Bearer') {
        // Return the header value as the auth header since it
        // could only be just the token
        // because kept getting error 
        // {"message":"Invalid key=value pair (missing equal-sign) 
        // in Authorization header (hashed with SHA-256 and encoded with Base64): 
        // 'tq+3z9YSC1l3AMICSj8OLNM1ReHx2MjT8RVZ5r+nStw='."}%                                                                                            
        return authHeader;
    }
